...
- 이미지11개를 기존 Supabase media 버킷 motion/approved-20260918/로 원본 그대로 업로드 완료, 다운로드 SHA256 11개 모두 일치. Base64·변환·재생성 없음.
- /motion/assets/의 기존 이미지 경로11개는 해당 Storage 원본으로 리다이렉트. 디자인/CSS/크롭 유지.
- 원본 이관용 임시 Edge Function ksop-asset-import: JWT + 임시 토큰 해시 + 30분 만료 + 파일명/크기/SHA256 제한, 기존 파일 덮어쓰기 불가. 이관 후 버전2로 교체해 모든 요청 410 응답으로 폐쇄 완료.
- 최신 Next 프로덕션 빌드 및 TypeScript 통과. 기존 DB 레코드/기존 파일 삭제 없음.
- checkpoint efff451 프리뷰 dpl_HdoH8EHYcwukQRMbtzAq3Es7pF6f 빌드 성공. 이미지 표시 확인. 프리뷰 DB 환경변수 미설정으로 공개 API503. 운영 배포는 환경변수4종 및 실제 DB 접근을 빌드 단계에서 확인하고 실패 시 운영 전환을 차단하도록 적용.
...
## 실제 운영 배포 및 검증 결과
- 운영 main 4d6ff13e56a1ba6a67c527e36dd70115576ec37f, Vercel dpl_CWMx2bLWScDLxSHxERQEzicvQEza READY, production alias 정상 반영.
- 운영 홈페이지 https://ksophomepage.vercel.app/ ; CMS https://ksophomepage.vercel.app/admin/login .
- 운영 설정/시리즈/이벤트/선수/뉴스/영상/파트너/소개 공개 API 모두200. 영상3·파트너6, 나머지 공개0(초안 유지). 미인증 관리자 API401.
- 브라우저 상세7개: 시리즈 일정, 시리즈별 이벤트, 시즌 랭킹, KSOP 뉴스, KSOP STUDIO, KSOP 소개, 함께하는 파트너 제목과 이동 확인. 상세 제목 번호 없음. 파트너 깨진 이미지0, 가로 넘침 없음.
- 운영 홈에 실제 영상3개 및 파트너6개 연결 확인. 영상 링크는 YouTube 외부 이동.
- DB 원본 이벤트25·기사2·설정11 유지. 기존 파일4 + 승인 이미지11 =15.
- 구형 Draft PR17 닫음. 커밋 기록/롤백 가능한 기존 배포 삭제 안 함.
- CMS 보안 로그인 요청 결과 user_took_over. 이후 로그인 폼만 확인, 인증 성공 증거 없음. 비밀번호를 읽거나 우회하지 않았음.
- 남은 필수 게이트: CMS 로그인 후 문구 저장→운영 홈페이지 반영→원문 복원, 이미지 업로드 및 각 등록 폼 실제 저장 점검. 이 검증 전 전체 완료 아님.
- 후속 작업은 main 최신부터 이어갈 것. 신규 프로젝트/DB 금지.
